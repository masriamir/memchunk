...
MemChunk *new_chunk( const uint32_t size )
{
    ...
    if ( !( mc = malloc( sizeof( *mc ) ) ) ) {
        fprintf( stderr,
                 "memchunk:new_memchunk - memchunk allocation failed\n" );
        return NULL;
    }
    ...
    if ( size > 0 ) {
        ...
        if ( !( mc->data = malloc( size ) ) ) {
            fprintf( stderr,
                     "memchunk:new_memchunk - data allocation failed\n" );
            mc->data = NULL;
        }
    } else {
        mc->data = NULL;
...
bool resize_chunk( MemChunk *mc, const uint32_t new_size )
{
    ...
    if ( !mc->data ) {
        fprintf( stderr,
                 "memchunk:resize_chunk - allocation of %u bytes failed\n",
                 new_size );
        return false;
    }
...
void seek_chunk( MemChunk *const mc, const uint32_t offset,
...
bool read_chunk_ofs( void *const buf, MemChunk *const mc, const uint32_t size,
...
bool write_chunk( MemChunk *mc, const void *data, const uint32_t size )
{
    ...
    /* resize the memchunk if trying to write past the end of the chunk */
    if ( mc->ptr + size > mc->size ) {
...
bool write_chunk_ofs( MemChunk *mc, const void *data, const uint32_t size,
                      const uint32_t offset )
{
